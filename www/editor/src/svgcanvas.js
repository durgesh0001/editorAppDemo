...
var textActions = canvas.textActions = function() {
  ...
  function setCursor(index) {
    ...
    var start_pt = ptToScreen(charbb.x, charbb.y ? charbb.y : textbb.y);
    var end_pt = ptToScreen(charbb.x, charbb.y ? charbb.y - charbb.height * 0.68 : (textbb.y + textbb.height));
  ...
  function setSelection(start, end, skipInput) {
    ...
    for (var charIndex = start; charIndex < end; charIndex++) {
      var curCharbb = chardata[charIndex];

      tl = ptToScreen(curCharbb.x, curCharbb.y - curCharbb.height * 0.75);
      tr = ptToScreen(curCharbb.x + curCharbb.width, curCharbb.y - curCharbb.height * 0.75);
      bl = ptToScreen(curCharbb.x, curCharbb.y + curCharbb.height * 0.25);
      br = ptToScreen(curCharbb.x + curCharbb.width, curCharbb.y + curCharbb.height * 0.25);
...
//        }
      ...
      var chardataHeight = textbb.height / textinput.value.split('\n').length;
      for(var i=0; i<len; i++) {
...
this.setFontSize = function(val) {
  ...
  $(selectedElements[0]).children('tspan').each(function() {
    $(this).attr('dy', '1.05em');
  });
};
    ...
    while (i--) {
      ...
      if (oldval !== String(newValue)) {
        if (attr == "#text") {
          ...
          /* start Dunice changes */
          var textLines = $('<tspan></tspan>').text(newValue).html().split('\n');
          var elemXPos = elem.getAttribute('x');
          var lineDelimiter = '.';
          var textHtml = '<tspan x="' + elemXPos + '">' + textLines[0]
            + '<tspan visibility="hidden">'+lineDelimiter+'</tspan></tspan>';
          var textLineIndex;

          for (textLineIndex = 1; textLineIndex < textLines.length; textLineIndex++) {
            textHtml += '<tspan x="'+elemXPos+'" dy="1.05em">' + textLines[textLineIndex]
              + '<tspan visibility="hidden">'+lineDelimiter+'</tspan></tspan>';
          }
        ...
        if (attr === 'x' && elem.tagName === 'text') {
          var newElemX = elem.getAttribute('x');
          $(elem).children('tspan').each(function(index) {
            this.setAttribute('x', newElemX);
          });
        }
        /* end Dunice changes */
